...
- `PROTOCOL`: http or https *(require)*.
- `DOMAIN`: your app domain *(optional)*.
- `WHITELIST`: allow domain in this variable from cors, separate with comma *(optional)*.
...
- `CLIENT_ID_GOOGLE`, `CLIENT_SECRET_GOOGLE`, `REDIRECT_URI_GOOGLE`: oauth google *(require)*.
- `CLIENT_ID_FACEBOOK`, `CLIENT_SECRET_FACEBOOK`, `REDIRECT_URI_FACEBOOK`: oauth facebook *(require)*.
...
- `NODE_ENV`: running environment (product, development, test) *(require)*.
...
| ----------- | -------- | ------- | -------- | ------- |
| Register | `POST /register` | <ul><li>`username`: [5-32] characters, only contain alphabet, dot and underscore *(string)*.</li><li>`password`: [6-32] characters, not contain white space *(string)*.</li><li>`firstName`, `lastName`: only contain alphabet characters, not contain 2 space in a row *(string)*.</li><li>`email`: must valid. ex: example@mail.com *(string)*.</li><li>`female`: 0 or 1 *(boolean)*.</li></ul> | data: <ul><li>`id`</li><li>`username`</li><li>`email`</li><li>`firstName`</li><li>`lastName`</li><li>`female`</li><li>`avatar`</li><li>`createdAt`</li><li>`updatedAt`</li></ul>`set-cookie`: token. | • all field in request.
| Login | `POST /login` | <ul><li>`username`: *(string)*.</li><li>`password`: *(string)*.</li></ul> | data: <ul><li>`id`</li><li>`username`</li><li>`email`</li><li>`firstName`</li><li>`lastName`</li><li>`female`</li><li>`avatar`</li><li>`createdAt`</li><li>`updatedAt`</li></ul>`set-cookie`: token. | • all field in request.
...
| Description | Endpoint | Request | Response | Require |
| ----------- | -------- | ------- | -------- | ------- |
| Update Information | `PATCH /info` | <ul><li>`firstName`, `lastName`: only contain alphabet characters, not contain 2 space in a row *(string)*.</li><li>`avatar`: only .jpg or .png *(file)*.</li></ul> | data: <ul><li>`id`</li><li>`username`</li><li>`email`</li><li>`firstName`</li><li>`lastName`</li><li>`female`</li><li>`avatar`</li><li>`createdAt`</li><li>`updatedAt`</li></ul> | • at least one field in request.<br>• logged.
...
| Get User | `GET /:username` | <ul></ul> | data: <ul><li>`id`</li><li>`username`</li><li>`email`</li><li>`firstName`</li><li>`lastName`</li><li>`female`</li><li>`avatar`</li><li>`createdAt`</li><li>`updatedAt`</li></ul> | 
| Get Friends | `GET /:username/friends` | <ul></li><li>`from`: *(query-number)*.</li><li>`limit`: *(query-number)*.</li></ul> | data: *array* <ul><li>`friend`</li><li>`status`</li><li>`User`: *relate to `friend`*<ul><li>`firstName`</li><li>`lastName`</li><li>`avatar`</li></ul></li><li>`createdAt`</li><li>`updatedAt`</li></ul> |
| Get Friend Requests | `GET /requests` | <ul></li><li>`from`: *(query-number)*.</li><li>`limit`: *(query-number)*.</li></ul> | data: *array*<ul><li>`from`</li><li>`status`</li><li>`User`: *relate to `from`*<ul><li>`firstName`</li><li>`lastName`</li><li>`avatar`</li></ul></li><li>`createdAt`</li><li>`updatedAt`</li></ul> | • logged.
