...
- `DB_NAME`: database *(require)*.
- `DB_USER`: database username *(require)*.
- `DB_PASS`: database password *(require)*.
- `DB_HOST`: database hosting *(require)*.
- `DB_DEV_NAME`, `DB_DEV_USER`, `DB_DEV_PASS`, `DB_DEV_HOST`: for development *(if not have these variable, running with development will fail).*
...
- `NODE_ENV=production npx sequelize-cli db:migrate` to initialize database.
- `npm start` to start the server (this project uses `NODE_ENV=production node src/server.js`).
